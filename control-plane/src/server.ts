  ...
  server.registerTool(
    'wm_create_document',
    {
      ...
      description:
        ...
        '(e.g. "topic", "workstream", "note"), an optional `slug`, optional `labels`, and an ' +
        'optional `spec` object holding the document body/fields. Returns the created document envelope.',
      inputSchema: {
        kind: z.string().describe('The document kind, e.g. "topic", "workstream", "note".'),
        ...
        spec: z
          ...
          .optional()
          .describe('Optional desired-state object (the document body/fields).'),
      },
    },
    async ({ kind, slug, labels, spec }) => asText(store.createDocument({ kind, slug, labels, spec })),
  );
